        ...
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                if(resultObj.getInteger("code") == 70000){
                   return new ApiResult<>(ResultVoEnum.Fail.getIndex(), "请求API错误："+resultObj.getString("sub_msg"));
                }
                JSONObject obj = JSONObject.parseObject(response.body()).getJSONObject("data");
                if(obj.getLong("count").intValue()==0)return new ApiResult<>(ResultVoEnum.DataError.getIndex(),"无订单可以更新");
...
//                    else if(result.getCode() == EnumResultVo.SUCCESS.getIndex()) addCount++;
            ...
            return new ApiResult<>(ResultVoEnum.Fail.getIndex(), "系统异常："+e.getMessage());
        }
