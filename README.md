...
启航电商ERP系统基于**至简快速开发平台**基础上二次开发而成，本项目目标是构建一套简单实用的电商ERP系统，主体功能包括：多平台订单管理、采购管理、库存管理、销售管理、客户管理、财务管理、物流管理、数据分析、进销存管理、统计报表，基本上覆盖了电商日常业务。本项目全部开源毫无保留给个人及企业免费使用，更适合中小电商企业。
...
+ 配置`application.yml`
  + redis（启动项目前先启动`redis`文件夹运行`start.bat`）
  + minio（进入`minio`文件夹启动minio）

+ 配置`application-druid.yml`
  + mysql
