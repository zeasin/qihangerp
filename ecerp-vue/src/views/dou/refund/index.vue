<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="该子订单购买的商品 sku_id" prop="comboId">
        <el-input
          ...
          placeholder="请输入该子订单购买的商品 sku_id"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      ...
      <el-form-item label="发货时间" prop="logisticsTime">
        <el-input
          v-model="queryParams.logisticsTime"
          placeholder="请输入发货时间"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <el-form-item label="父订单总金额 (单位: 分) 即用户实际支付金额, 不包含运费" prop="orderAmount">
        <el-input
          ...
          placeholder="请输入父订单总金额 (单位: 分) 即用户实际支付金额, 不包含运费"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="金额" prop="comboAmount">
        <el-input
          v-model="queryParams.comboAmount"
          placeholder="请输入金额"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <el-form-item label="erp商品规格ID" prop="erpSpecId">
        <el-input
          v-model="queryParams.erpSpecId"
          placeholder="请输入erp商品规格ID"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="refundList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="售后类型，枚举为0(退货退款),1(已发货仅退款),2(未发货仅退款),3(换货)" align="center" prop="aftersaleType" />
      <el-table-column label="抖音订单id" align="center" prop="orderId" />
      <el-table-column label="抖音子订单id" align="center" prop="subOrderId" />
      <el-table-column label="订单所属商户id" align="center" prop="shopId" />
      ...
      <el-table-column label="父订单总金额 (单位: 分) 即用户实际支付金额, 不包含运费" align="center" prop="orderAmount" />
      <el-table-column label="金额" align="center" prop="comboAmount" />
      ...
      <el-table-column label="确认状态0:未处理2已签收9供应商已退款" align="center" prop="auditStatus" />
      <el-table-column label="审核时间" align="center" prop="auditTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.auditTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="枚举为6(待商家同意),7(待买家退货),11(待商家二次同意),12(售后成功),13(换货待买家收货),14(换货成功),27(商家一次拒绝),28(售后失败),29(商家二次拒绝)" align="center" prop="refundStatus" />
...
<script>
import { listRefund, getRefund, delRefund, addRefund, updateRefund } from "@/api/dou/refund";
...
export default {
  ...
  methods: {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('dou/refund/export', {
        ...this.queryParams
