package com.qihang.erp.api.mapper;

import com.qihang.erp.api.domain.WmsStockOutEntryItemDetail;
import com.baomidou.mybatisplus.core.mapper.BaseMapper;

/**
* @author TW
* @description 针对表【wms_stock_out_entry_item_detail(出库明细详情)】的数据库操作Mapper
* @createDate 2024-04-26 11:31:15
* @Entity com.qihang.erp.api.domain.WmsStockOutEntryItemDetail
*/
public interface WmsStockOutEntryItemDetailMapper extends BaseMapper<WmsStockOutEntryItemDetail> {

}




